...
def handle_actuation(action):
    try:
        ...
        if action.get('fan_action') == 'turn_on_fan':
            grovepi.digitalWrite(fan_led, 1)
        elif action.get('fan_action') == 'turn_off_fan':
            grovepi.digitalWrite(fan_led, 0)

        if action.get('hum_action') == 'turn_on_dehumidifier':
            grovepi.digitalWrite(fan_led, 1)
        elif action.get('hum_action') == 'turn_off_dehumidifier':
            grovepi.digitalWrite(fan_led, 0)
    except Exception as e:
